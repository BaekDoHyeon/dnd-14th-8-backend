 ...
 * API마다 최소 비용 호출 1건씩만 수행한다 (MapGlot/ODsay는 일일 쿼터 소모).
 */
@Slf4j
@Service
@Profile("local")
...
public class ExternalApiHealthServiceImpl implements ExternalApiHealthService {
    ...
    public ExternalApiHealthResponse checkExternalApis() {
        List<ApiCheckResult> results = List.of(
                ...
                check("MapGlot Route", this::checkMapGlotRoute)
        );
    ...
    private String checkMapGlotRoute() {
        DrivingRouteResult result = mapGlotRouteClient.calculateDriving(
                CITY_HALL_LAT, CITY_HALL_LNG, GANGNAM_STATION_LAT, GANGNAM_STATION_LNG);
        if (!result.reachable()) {
            throw new IllegalStateException("자동차 경로 응답 없음 (로그에서 원인 확인)");
        }
        return "시청→강남역 MapGlot 자동차 경로 조회 성공: " + result.durationSeconds() / 60 + "분";
    }
}
